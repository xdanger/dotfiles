...
→ `algorithmic-art` with a slider for occupation probability *p*, N×N grid with flood-fill coloring connected clusters. The user moves *p* from 0.4 → 0.7 and **sees** the giant cluster pop into existence around p_c ≈ 0.5927. This is the canonical p5 case — no prose can deliver that "aha".
