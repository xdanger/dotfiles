...
3. 使用 `gh` 命令行工具查找对应的 issue/PR 的上下文：
   - 如果你看到的 `#$ARGUMENTS` 为空，则忽略本条指令，继续第 4 条指令
   - 如果 `#$ARGUMENTS` 是一个 issue/PR 的编号（例如 `#123`），则使用 `gh` 查找该条 issue/PR，获得其上下文
     - 如果没有找到对应的 issue/PR，则中断执行，提示我没有找到对应的 issue/PR
     - 如果找到了，则将 `#$ARGUMENTS` 作为 commit message 的 `scope`
4. 使用 gitmoji 来标准化描述所做的更改
5. 在传统 commit 信息的基础上：
